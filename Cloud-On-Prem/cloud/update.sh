...
resolve_cloud_app_dir() {
  ...
  # Enforce scoped runtime path for all new installs and updates.
  echo "${SCOPED_APP_DIR}"
}
...
migrate_legacy_cloud_layout_if_needed() {
  ...
  echo "ℹ️  Migrating legacy runtime layout: ${LEGACY_APP_DIR} -> ${SCOPED_APP_DIR}"
  ...
  for item in "${items[@]}"; do
    if [ -e "${LEGACY_APP_DIR}/${item}" ] && [ ! -e "${SCOPED_APP_DIR}/${item}" ]; then
      mv "${LEGACY_APP_DIR}/${item}" "${SCOPED_APP_DIR}/${item}"
    fi
  done
...
# Full schema replay/pruning in automated updates is disabled permanently for data safety.
...
if [ "${REQUESTED_FULL_SCHEMA_RECONCILIATION}" = "true" ]; then
  ...
  echo "   Use explicit offline DBA procedures for schema reconciliation."
  exit 1
fi
...
get_db_owner() {
  local db_owner=""
  if [ -f "$ENV_FILE" ]; then
    db_owner="$(grep -E '^DB_USER=' "$ENV_FILE" 2>/dev/null | tail -1 | cut -d= -f2- | tr -d '"' | tr -d "'" || true)"
    if [ -z "$db_owner" ]; then
      local db_url
      db_url="$(grep -E '^DATABASE_URL=' "$ENV_FILE" 2>/dev/null | tail -1 | cut -d= -f2- | tr -d '"' | tr -d "'" || true)"
      if [ -n "$db_url" ]; then
        db_owner="$(printf '%s' "$db_url" | sed -E 's|^[a-zA-Z0-9+.-]+://([^:/@]+).*|\1|' || true)"
      fi
    fi
  fi
  if [ -z "$db_owner" ]; then
...
get_db_name() {
  local db_name=""
  if [ -f "$ENV_FILE" ]; then
    db_name="$(grep -E '^DB_NAME=' "$ENV_FILE" 2>/dev/null | tail -1 | cut -d= -f2- | tr -d '"' | tr -d "'" || true)"
    if [ -z "$db_name" ]; then
      local db_url
      db_url="$(grep -E '^DATABASE_URL=' "$ENV_FILE" 2>/dev/null | tail -1 | cut -d= -f2- | tr -d '"' | tr -d "'" || true)"
      if [ -n "$db_url" ]; then
        db_name="$(printf '%s' "$db_url" | sed -E 's|^[a-zA-Z0-9+.-]+://[^/]+/([^?]+).*$|\1|' || true)"
      fi
    fi
  fi
  if [ -z "$db_name" ]; then
...
AUTO_YES=false
ROLLBACK_DB=false
DB_OWNER="$(get_db_owner)"
APP_DB_NAME="$(get_db_name)"
APP_DB_URL="$(get_db_url)"
UPDATE_COMPONENT="all"
UPDATE_APP_PORT="3000"
UPDATE_TX_ID="cloud-${TIMESTAMP}-$RANDOM"
...
database_size_bytes() {
  local bytes
  bytes="$(sudo -u postgres psql -d postgres -tAc "SELECT pg_database_size('${APP_DB_NAME}');" 2>/dev/null | tr -d '[:space:]' || true)"
  if [[ "$bytes" =~ ^[0-9]+$ ]]; then
    echo "$bytes"
  else
    echo "0"
...
pg_dump_with_progress() {
  ...
  sudo -u postgres pg_dump "$APP_DB_NAME" > "$output_file" 2>/dev/null &
  dump_pid=$!
...
ensure_platform_superadmin() {
  ...
  existing_count="$(sudo -u postgres psql -d "$APP_DB_NAME" -tAc "SELECT COUNT(*) FROM users WHERE lower(email)=lower('${superadmin_email}')" 2>/dev/null | tr -d '[:space:]' || echo "0")"
  ...
  if sudo -u postgres psql -d "$APP_DB_NAME" -v ON_ERROR_STOP=1 >/dev/null 2>&1 <<EOSQL
DO \$\$
DECLARE
...
validate_runtime_identity_for_update() {
  ...
  if [ -n "$marker_product" ] && [ "$marker_product" != "cloud" ]; then
    fail "Runtime identity mismatch: marker product is '$marker_product', expected 'cloud'"
    return 1
  fi
  return 0
}
...
write_release_provenance() {
  local now version build_date manifest_version manifest_min
  now="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
  version="$(get_version_from "$APP_DIR")"
...
apply_runtime_space_optimization() {
  ...
  if [ -d "${APP_DIR}/releases" ]; then
    ...
    mapfile -t release_archives < <(ls -1t "${APP_DIR}"/releases/LP-CL-V*.tar.gz 2>/dev/null || true)
  ...
  find /tmp /var/tmp -maxdepth 1 -mindepth 1 -type d -name 'dist-cloud*' -mtime +"$staging_days" -exec rm -rf {} + 2>/dev/null || true
...
ensure_cloud_sync_secret_configured() {
  if [ ! -f "$ENV_FILE" ]; then
    return 0
  fi
  local sync_secret=""
  ...
  if [ -n "${env_sync_secret:-}" ]; then
    set_env_value "ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD" "${env_sync_secret}"
    ok "Set ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD from install/update environment"
    return 0
  fi
  warn "ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD is not configured. Shared-mode bootstrap auth for on-prem check-in recovery will be unavailable; per-system sync credentials remain supported."
  return 0
}
...
ensure_required_supported_languages() {
  ...
  if sudo -u postgres psql -d "$APP_DB_NAME" -v ON_ERROR_STOP=1 >>"$UPDATE_LOG" 2>&1 <<'EOSQL_LANGUAGES'; then
DO $$
BEGIN
  WITH ranked AS (
...
ensure_pm2_available() {
  ...
  ensure_pm2_symlink_from_nvm() {
    nvm_pm2="$(find "/home/${APP_USER}/.nvm/versions/node" -type f -path '*/bin/pm2' 2>/dev/null | sort | tail -1 || true)"
    if [ -n "$nvm_pm2" ] && [ -x "$nvm_pm2" ]; then
      ln -sfn "$nvm_pm2" /usr/local/bin/pm2 2>/dev/null || true
      return 0
    fi
    return 1
  }

  echo -n "  Checking PM2 runtime...         "
  if sudo -u "$APP_USER" bash -lc "command -v pm2 >/dev/null 2>&1"; then
    ok "PM2 available"
    return 0
  fi

  if ensure_pm2_symlink_from_nvm && sudo -u "$APP_USER" bash -lc "command -v pm2 >/dev/null 2>&1"; then
    ok "PM2 available (linked from nvm)"
    return 0
  fi

  warn "PM2 not found for app user '$APP_USER' — attempting install"
  if ! command -v npm >/dev/null 2>&1; then
    fail "npm is required to install PM2 automatically"
    return 1
  fi

  if npm install -g pm2 >> "$UPDATE_LOG" 2>&1; then
    ensure_pm2_symlink_from_nvm || true
    if sudo -u "$APP_USER" bash -lc "command -v pm2 >/dev/null 2>&1"; then
      ok "PM2 installed"
      return 0
    fi
  ...
  fail "PM2 install failed; ensure PM2 is installed and in $APP_USER PATH"
  return 1
}
...
health_probe() {
  ...
  local url
  local ports=("$primary_port" "3000" "5000" "3001")
  local seen=" "
  local p
  for p in "${ports[@]}"; do
    [ -z "$p" ] && continue
...
capture_pm2_diagnostics() {
  {
    echo ""
    echo "===== PM2 DIAGNOSTICS ====="
    echo "Timestamp: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
    sudo -u "$APP_USER" bash -lc "pm2 describe \"$APP_NAME\"" 2>&1 || true
    echo ""
    sudo -u "$APP_USER" bash -lc "pm2 logs \"$APP_NAME\" --lines 120 --nostream" 2>&1 || true
    echo "===== END PM2 DIAGNOSTICS ====="
    echo ""
  } >> "$UPDATE_LOG" 2>&1
}
...
run_psql_sql() {
  ...
  if [ "$mode" = "capture" ]; then
    sudo -u postgres psql -d "$APP_DB_NAME" -v ON_ERROR_STOP=1 -At -c "$sql"
  else
    sudo -u postgres psql -d "$APP_DB_NAME" -v ON_ERROR_STOP=1 -c "$sql" >> "$UPDATE_LOG" 2>&1
  fi
}
...
run_psql_file() {
  ...
  sudo -u postgres psql -d "$APP_DB_NAME" -v ON_ERROR_STOP=1 -f "$sql_file" >> "$UPDATE_LOG" 2>&1
}
...
run_psql_file_allow_duplicate_ddl() {
  ...
  if [ -n "$db_url" ]; then
    ...
    sudo -u postgres psql -d "$APP_DB_NAME" -v ON_ERROR_STOP=0 -f "$sql_file" >"$output_file" 2>&1 || rc=$?
  fi
...
EXPECTED_CONTRACT_PRODUCT="cloud"
...
prune_non_contract_columns() {
  ...
  while IFS= read -r key; do
    ...
    # Skip tables that are not contract-owned (handled by object archival).
...
archive_non_contract_public_objects() {
  ...
  while IFS= read -r obj; do
    ...
    # Type relocation can fail if still referenced by canonical objects; do not hard-fail.
...
update_lppadmin_command() {
  ...
  local local_devadmin_backup="$BACKUP_DIR/devadmin.sh.pre-update.${TIMESTAMP}"
  ...
  [ -e /usr/local/bin/devadmin ] && cp -a /usr/local/bin/devadmin "$local_devadmin_backup" 2>/dev/null || true
  ...
  if [ -f "$DIST_DIR/scripts/devadmin.sh" ]; then
    install -m 755 "$DIST_DIR/scripts/devadmin.sh" /antigravity/devadmin.sh 2>/dev/null || true
    ln -sfn /antigravity/devadmin.sh /usr/local/bin/devadmin 2>/dev/null || true
    ok "Updated /usr/local/bin/devadmin symlink"
  fi
...
if [ -n "$APP_DB_URL" ] && psql "$APP_DB_URL" -c "SELECT 1" > /dev/null 2>&1; then
  ok "PostgreSQL is reachable (DATABASE_URL)"
elif sudo -u postgres psql -d "$APP_DB_NAME" -c "SELECT 1" > /dev/null 2>&1; then
  ok "PostgreSQL is reachable"
else
  if [ -n "$APP_DB_URL" ]; then
    fail "Cannot connect using DATABASE_URL (fallback DB name '$APP_DB_NAME' also failed)"
  else
    fail "Cannot connect to database '$APP_DB_NAME'"
  fi
  PREFLIGHT_OK=false
fi
...
if [ -f "$MANIFEST_FILE" ]; then
  ...
  if [ "$MANIFEST_PRODUCT" != "cloud" ]; then
    fail "Package product '$MANIFEST_PRODUCT' is not valid for cloud runtime"
    PREFLIGHT_OK=false
    ...
    ok "Manifest valid (product=cloud, minSupported=${MANIFEST_MIN})"
  fi
else
...
if [ -d "$DIST_DIR/migrations" ]; then
  ...
  while IFS= read -r sql_file; do
    ...
    case "$(basename "$sql_file")" in
      # Cloud runtime intentionally excludes this on-prem-only migration.
      0052_add_cust_super_role.sql) continue ;;
    esac
...
DATA_PARITY_BASELINE_FILE="/tmp/learnplay-cloud-data-parity-baseline-${TIMESTAMP}.json"
...
if [ -f "$ENV_FILE" ]; then
  echo -n "  Enforcing cloud deployment mode... "
  set_env_value "PLATFORM_ENV" "cloud"
  set_env_value "DEPLOYMENT_MODE" "cloud"
  set_env_value "ONPREM_MODE" "false"
  set_env_value "ONPREM_OWN_API_KEYS" "false"
  ok "Done"
  ensure_cloud_sync_secret_configured
fi
...
if [ "$SKIP_INTERNAL_DEV_DIFF_CHECK" = true ]; then
  ...
  if sudo -u "$APP_USER" bash -c '
    node "'"$APP_DIR"'/scripts/migration-governance.mjs" validate --deployment-mode cloud
  ' >> "$UPDATE_LOG" 2>&1; then
...
if sudo -u "$APP_USER" bash -c '
  ...
  if [ -f "'"$APP_DIR"'/.env" ]; then . "'"$APP_DIR"'/.env"; fi
  set +a
  DEPLOYMENT_MODE=cloud node "'"$APP_DIR"'/scripts/migrate.js"
' >> "$UPDATE_LOG" 2>&1; then
...
if [ "$CRITICAL_FAILURE" != true ]; then
  if [ "$SKIP_INTERNAL_DEV_DIFF_CHECK" = true ]; then
    ...
    if sudo -u "$APP_USER" bash -c '
      ...
      if [ -f "'"$APP_DIR"'/.env" ]; then . "'"$APP_DIR"'/.env"; fi
      set +a
      node "'"$APP_DIR"'/scripts/migration-governance.mjs" verify-runtime-contract --deployment-mode cloud --db-url "${DATABASE_URL:-}" --contract-schema-file "'"$APP_DIR"'/schema-full.sql" --functional-only
    ' >> "$UPDATE_LOG" 2>&1; then
      ok "Verified"
    else
...
info "Schema reconciliation/pruning legacy path remains disabled in automated update flow for data safety."
...
HEALTH_TIMEOUT_SECONDS="${LEARNPLAY_HEALTH_TIMEOUT_SECONDS:-120}"
HEALTH_INTERVAL_SECONDS=3
HEALTH_ATTEMPTS=$((HEALTH_TIMEOUT_SECONDS / HEALTH_INTERVAL_SECONDS))
if [ "$HEALTH_ATTEMPTS" -lt 1 ]; then
  HEALTH_ATTEMPTS=1
fi

for i in $(seq 1 "$HEALTH_ATTEMPTS"); do
  if health_probe "$UPDATE_APP_PORT"; then
    HEALTH_OK=true
    break
  fi
  echo -n "."
...
if [ "$HEALTH_OK" = true ]; then
  ...
  fail "Health check failed after ${HEALTH_TIMEOUT_SECONDS} seconds"
...
if [ "$HEALTH_OK" = true ]; then
  ...
  if [ "$UPDATE_COMPONENT" = "all" ] || [ "$UPDATE_COMPONENT" = "app-db" ]; then
    step "$COMPLETE_STEP" "Updating lppadmin Command"
    info "Component mode: ${UPDATE_COMPONENT} (ensuring lppadmin command/scripts are refreshed)"
    update_lppadmin_command || exit 1
    log "lppadmin command/scripts updated as part of package update"
    COMPLETE_STEP=$((COMPLETE_STEP + 1))
  fi
  ...
  if [ "$ROLLBACK_DB" = true ] && [ -f "$BACKUP_PATH/database_${TIMESTAMP}.sql" ]; then
    ...
    sudo -u postgres dropdb "$APP_DB_NAME" 2>/dev/null || true
    sudo -u postgres psql -tAc "SELECT 1 FROM pg_roles WHERE rolname='${DB_OWNER}'" | grep -q 1 || DB_OWNER="learnplay"
    sudo -u postgres createdb -O "$DB_OWNER" "$APP_DB_NAME" 2>/dev/null || sudo -u postgres createdb "$APP_DB_NAME" 2>/dev/null || true
    sudo -u postgres psql -d "$APP_DB_NAME" < "$BACKUP_PATH/database_${TIMESTAMP}.sql" >> "$UPDATE_LOG" 2>&1
    ok "Database restored from backup"
  else
    warn "Database was NOT rolled back (no backup available)"
    info "Manual restore: psql $APP_DB_NAME < $BACKUP_PATH/database_${TIMESTAMP}.sql"
  fi
  ...
  # Re-resolve app port during rollback in case Step 9 never set UPDATE_APP_PORT.
  if [ -f "$APP_DIR/.env" ]; then
    UPDATE_APP_PORT="$(grep -E "^PORT=" "$APP_DIR/.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"' | tr -d "'" || true)"
  fi
  UPDATE_APP_PORT="${UPDATE_APP_PORT:-3000}"
