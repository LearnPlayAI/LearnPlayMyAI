...
is_protected_runtime_key() {
  ...
  case "$key" in
    DEPLOYMENT_MODE|PLATFORM_ENV|ONPREM_MODE|SYSTEM_TYPE|COOKIE_NAME|COOKIE_DOMAIN|SESSION_COOKIE_NAME|SESSION_COOKIE_DOMAIN|APP_VARIANT|RUNTIME_VARIANT)
      return 0
      ;;
    *)
      return 1
...
app_pm2_process_online() {
  ...
  plist="$(sudo -u "$u" env HOME="$home" PM2_HOME="$home/.pm2" pm2 jlist 2>/dev/null | tr -d '\n' || true)"
...
install_global_command() {
  ...
  if [ -f "${SCOPE_SCRIPTS}/devadmin.sh" ]; then
    run_cmd_step "Install/update /usr/local/bin/devadmin (cloud-only)" install -m 755 "${SCOPE_SCRIPTS}/devadmin.sh" /antigravity/devadmin.sh || return 1
    run_cmd_step_sh "Install /usr/local/bin/devadmin symlink (cloud-only)" "ln -sfn '/antigravity/devadmin.sh' '/usr/local/bin/devadmin'" || return 1
  fi
...
find_latest_update_tarball() {
  local latest=""
  if [ "$SCOPE" = "onprem" ]; then
    latest="$(ls -1t /tmp/learnplay-onprem*.tar.gz 2>/dev/null | head -1 || true)"
  else
    latest="$(ls -1t /tmp/learnplay-cloud*.tar.gz 2>/dev/null | head -1 || true)"
  fi
  if [ -z "$latest" ]; then
