...
LEARNPLAY_SCOPE="${LEARNPLAY_SCOPE:-cloud}"
...
# Force fully non-interactive apt operations during installer execution.
...
apt-get update -qq
# Keep base packages minimal and only for installer/runtime prerequisites:
# - curl/gnupg2/lsb-release/ca-certificates: trusted apt repos for Node/Postgres
# - build-essential: native npm modules during production install
apt-get install -y -qq curl gnupg2 build-essential lsb-release ca-certificates
