...
APP_NAME="learnplay"
SCOPED_APP_DIR="/opt/learnplay/cloud"
resolve_cloud_app_dir() {
...
APP_DIR="$(resolve_cloud_app_dir)"
UPLOAD_DIR="${LEARNPLAY_UPLOAD_DIR:-${APP_DIR}/uploads}"
...
INSTALL_TX_ID="cloud-install-$(date +%Y%m%d_%H%M%S)-$RANDOM"
RUNTIME_MARKER_FILE="${APP_DIR}/.runtime-identity.json"
PROVENANCE_FILE="${APP_DIR}/.release-provenance.json"
APP_SERVICE_NAME="${APP_SERVICE_NAME:-learnplay-cloud}"
...
resolve_dist_dir() {
  ...
  # 5. Try glob for renamed dist-cloud dirs in /tmp
...
DIST_DIR="$(resolve_dist_dir)" || {
  ...
  echo "      export LEARNPLAY_DIST_DIR=/path/to/dist-cloud"
  echo "      Then re-run this script."
  echo ""
  echo "   2. Re-extract the distribution package:"
  echo "      cd /tmp && unzip dist-cloud.zip"
  echo ""
  exit 1
}
...
TUNING
  ...
  cat > "$unit_file" <<UNIT
[Unit]
Description=LearnPlay Cloud Service
...
stop_pm2_app_runtime() {
  if sudo -u "$APP_USER" bash -lc "command -v pm2 >/dev/null 2>&1" 2>/dev/null; then
    sudo -u "$APP_USER" bash -lc "pm2 stop \"$APP_NAME\" --silent >/dev/null 2>&1 || true"
    sudo -u "$APP_USER" bash -lc "pm2 delete \"$APP_NAME\" >/dev/null 2>&1 || true"
    sudo -u "$APP_USER" bash -lc "pm2 stop \"$APP_SERVICE_NAME\" --silent >/dev/null 2>&1 || true"
    sudo -u "$APP_USER" bash -lc "pm2 delete \"$APP_SERVICE_NAME\" >/dev/null 2>&1 || true"
    sudo -u "$APP_USER" bash -lc "pm2 save --force >/dev/null 2>&1 || true"
  fi
}
...
require_value_non_interactive() {
  local key_name="$1"
  local key_value="$2"
  if is_non_interactive && [ -z "$key_value" ]; then
    log "   ❌ ${key_name} is required in non-interactive mode"
    exit 1
  fi
}

verify_non_transactional_seed_data() {
  local data_dir="$APP_DIR/data"
  local failures=()
  local file table expected_rows actual_rows

  if [ ! -d "$data_dir" ]; then
    log "   ❌ Seed verification failed: data directory missing at $data_dir"
    return 1
  fi
...
PY
    ...
    if ! [[ "$expected_rows" =~ ^[0-9]+$ ]] || [ "$expected_rows" -eq 0 ]; then
      continue
    fi

    actual_rows="$(psql "$DATABASE_URL" -t -A -c "SELECT COUNT(*) FROM \"$table\";" 2>/dev/null || echo "__ERR__")"
    if [ "$actual_rows" = "__ERR__" ] || ! [[ "$actual_rows" =~ ^[0-9]+$ ]] || [ "$actual_rows" -lt "$expected_rows" ]; then
      failures+=("$table")
    fi
  done

  # Bootstrap entities required for system operation.
  [ "$(psql "$DATABASE_URL" -t -A -c "SELECT 1 FROM users WHERE lower(email)=lower('admin@learnplay.co.za') AND \"isSuperAdmin\"=true LIMIT 1;" 2>/dev/null || true)" = "1" ] || failures+=("users(superadmin)")
  [ "$(psql "$DATABASE_URL" -t -A -c "SELECT 1 FROM users WHERE lower(email)=lower('${SQL_ADMIN_EMAIL}') LIMIT 1;" 2>/dev/null || true)" = "1" ] || failures+=("users(org_admin)")
  [ "$(psql "$DATABASE_URL" -t -A -c "SELECT 1 FROM organizations WHERE name='${SQL_ORG_NAME}' LIMIT 1;" 2>/dev/null || true)" = "1" ] || failures+=("organizations(first_org)")
  [ "$(psql "$DATABASE_URL" -t -A -c "SELECT 1 FROM \"userOrganizationRoles\" uor JOIN users u ON u.id=uor.\"userId\" JOIN organizations o ON o.id=uor.\"organizationId\" WHERE lower(u.email)=lower('${SQL_ADMIN_EMAIL}') AND o.name='${SQL_ORG_NAME}' AND uor.role='org_admin' LIMIT 1;" 2>/dev/null || true)" = "1" ] || failures+=("userOrganizationRoles(org_admin)")
  [ "$(psql "$DATABASE_URL" -t -A -c "SELECT 1 FROM \"brandingThemes\" WHERE \"organizationId\" IS NULL LIMIT 1;" 2>/dev/null || true)" = "1" ] || failures+=("brandingThemes(platform)")
  [ "$(psql "$DATABASE_URL" -t -A -c "SELECT 1 FROM \"brandingThemes\" bt JOIN organizations o ON o.id=bt.\"organizationId\" WHERE o.name='${SQL_ORG_NAME}' LIMIT 1;" 2>/dev/null || true)" = "1" ] || failures+=("brandingThemes(org)")

  if [ "${#failures[@]}" -gt 0 ]; then
    log "   ❌ Seed verification failed. Missing/incomplete: ${failures[*]}"
    return 1
  fi

  log "   ✅ Seed verification passed: non-transactional baseline data present"
  return 0
}
...
normalize_env_secret_multiline() {
  local value="$1"
  value="${value//$'\r'/}"
  while [ "${value#"$'\n'"}" != "$value" ]; do value="${value#"$'\n'"}"; done
  while [ "${value%"$'\n'"}" != "$value" ]; do value="${value%"$'\n'"}"; done
  # Store multiline secrets as escaped \n for dotenv.
  value="${value//$'\n'/\\n}"
  printf '%s' "$value"
}

collect_cloud_license_private_key() {
  local key_input=""
  local line=""

  read -r -s -p "  CLOUD_LICENSE_PRIVATE_KEY: " key_input
  echo ""

  if [[ "$key_input" == file:* ]]; then
    local key_file="${key_input#file:}"
    if [ -f "$key_file" ]; then
  ...
  elif [[ "$key_input" == /* ]] && [ -f "$key_input" ]; then
    key_input="$(cat "$key_input")"
  fi

  if [[ "$key_input" == *"-----BEGIN"* && "$key_input" != *"-----END"* ]]; then
    echo "  Detected PEM header. Paste remaining lines, then press Enter after the END line."
  ...
  key_input="$(normalize_env_secret_multiline "$key_input")"
  if [ -z "$key_input" ]; then
    return 1
  fi

  CLOUD_LICENSE_PRIVATE_KEY="$key_input"
  return 0
}
...
if [ "$EXISTING_INSTALL" = true ]; then
  ...
  if [ -n "${LEARNPLAY_REINSTALL:-}" ]; then
    ...
    log "   ❌ Existing installation detected in non-interactive mode without LEARNPLAY_REINSTALL=yes"
    exit 1
  else
  ...
  if command -v pm2 &>/dev/null; then
    ...
    sudo -u "$APP_USER" bash -lc "pm2 stop all" 2>/dev/null || true
    sudo -u "$APP_USER" bash -lc "pm2 delete all" 2>/dev/null || true
...
DROPALL
  ...
  # 4. Remove application files (keep .env backup for reference)
  ...
  rm -rf "$APP_DIR/server" "$APP_DIR/client" "$APP_DIR/migrations" "$APP_DIR/scripts" \
         ...
         "$APP_DIR/version.json" "$APP_DIR/ecosystem.config.cjs" "$APP_DIR/data" \
         "$APP_DIR/.env"
...
chown -R "$APP_USER:$APP_USER" "$APP_DIR" /opt/learnplay/keys 2>/dev/null || true
...
if [ -f /tmp/learnplay-db-credentials ]; then
  ...
  if is_non_interactive; then
    log "   ❌ Missing /tmp/learnplay-db-credentials in non-interactive mode"
    exit 1
  fi
...
if [ -n "${LEARNPLAY_DOMAIN:-}" ]; then
  ...
  log "   ❌ LEARNPLAY_DOMAIN is required in non-interactive mode"
  exit 1
else
...
if [ -n "${LEARNPLAY_ADMIN_EMAIL:-}" ]; then
  ...
  log "   ❌ LEARNPLAY_ADMIN_EMAIL is required in non-interactive mode"
  exit 1
else
...
YOCO_WEBHOOK_SECRET=""
CLOUD_LICENSE_PRIVATE_KEY="${LEARNPLAY_CLOUD_LICENSE_PRIVATE_KEY:-}"
CLOUD_LICENSE_PRIVATE_KEY="$(normalize_env_secret_multiline "$CLOUD_LICENSE_PRIVATE_KEY")"
require_value_non_interactive "LEARNPLAY_CLOUD_LICENSE_PRIVATE_KEY" "$CLOUD_LICENSE_PRIVATE_KEY"

if [ -z "$CLOUD_LICENSE_PRIVATE_KEY" ]; then
  while ! collect_cloud_license_private_key; do
    echo -e "${RED}  CLOUD_LICENSE_PRIVATE_KEY is required.${NC}"
  done
fi
...
if [ -z "$ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD" ]; then
  echo ""
  echo -e "${BOLD}On-Prem Cloud PRD Signing Secret (Optional)${NC}"
  echo -e "${DIM}Legacy bootstrap-only. Per-system sync credentials are preferred and auto-provisioned after authenticated linking.${NC}"
  if ! is_non_interactive; then
...
echo -e "${DIM}The OS user who will own and run the LearnPlay application.${NC}"
echo ""

DEFAULT_ADMIN_USER="$APP_USER"
if is_non_interactive; then
  LP_ADMIN_USER="${LEARNPLAY_ADMIN_USER:-$DEFAULT_ADMIN_USER}"
else
  read -p "  Admin username [$DEFAULT_ADMIN_USER]: " INPUT_ADMIN_USER
  LP_ADMIN_USER="${INPUT_ADMIN_USER:-$DEFAULT_ADMIN_USER}"
fi
...
echo "  2) Production   — For live use. Learner registration requires a valid production license."
echo ""
...
if [ "$SYSTEM_TYPE" = "development" ]; then
  echo ""
  echo -e "${YELLOW}   Note: Development systems never allow learner self-registration.${NC}"
fi
...
if lsof -i :"$APP_PORT" -sTCP:LISTEN -t > /dev/null 2>&1; then
  ...
  if is_non_interactive; then
    log "   ❌ Port $APP_PORT already in use in non-interactive mode"
    exit 1
  fi
...
cat > "$APP_DIR/.env" << EOF
# ============================================
# LearnPlay Cloud (Linux) Configuration
...
# URLs (updated to https:// automatically when SSL is enabled via lppadmin ssl)
BASE_URL=$BASE_URL
PUBLIC_BASE_URL=$BASE_URL
...
CLOUD_LICENSE_PRIVATE_KEY=$CLOUD_LICENSE_PRIVATE_KEY
ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD=$ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD

# Payment Gateway
PAYMENT_GATEWAY_ENABLED=${PAYMENT_GATEWAY_ENABLED:-true}
...
# Validate critical keys persisted correctly in .env
for required_key in CLOUD_LICENSE_PRIVATE_KEY; do
  if ! grep -qE "^${required_key}=.+" "$APP_DIR/.env"; then
    log "   ❌ Required key ${required_key} missing from .env after generation"
    exit 1
  fi
done
...
cp "$DIST_DIR/runtime-assets-manifest.txt" "$APP_DIR/" 2>/dev/null || true
...
# Runtime uploads for production installs live under ${APP_DIR}/uploads.
if [ -d "$DIST_DIR/uploads/public" ]; then
  mkdir -p "$UPLOAD_DIR"
  cp -r "$DIST_DIR/uploads/public" "$UPLOAD_DIR/"
fi
mkdir -p "$UPLOAD_DIR/private"
chown -R "$APP_USER:$APP_USER" "$UPLOAD_DIR" 2>/dev/null || true

# Ensure required production directories exist even on fresh hosts.
mkdir -p "$BACKUP_DIR"
...
# ============================================
# Step 5a: Validate Required Runtime Assets
# ============================================
log "🔎 Validating required runtime assets..."
...
if [ ! -f "$APP_DIR/runtime-assets-manifest.txt" ]; then
  MANIFEST_MISSING=true
  log "   ⚠️  Missing runtime asset manifest at $APP_DIR/runtime-assets-manifest.txt"
  log "   ℹ️  Generating fallback manifest from current upload assets"
  {
    echo "# Auto-generated fallback manifest ($(date -u +%Y-%m-%dT%H:%M:%SZ))"
    find "$UPLOAD_DIR/public" -type f 2>/dev/null | sed "s|^$UPLOAD_DIR/public/||" | sort
  } > "$APP_DIR/runtime-assets-manifest.txt" || true
fi

SEED_COUNT=$(count_files_if_dir "$APP_DIR/data")
STYLE_COUNT=$(count_files_if_dir "$UPLOAD_DIR/public/gamma/image-styles")
THEME_COUNT=$(count_files_if_dir "$UPLOAD_DIR/public/gamma/themes")
BRANDING_COUNT=$(count_files_if_dir "$UPLOAD_DIR/public/branding/platform")
BUNDLED_PUBLIC_COUNT=$(count_files_if_dir "$DIST_DIR/uploads/public")

if [ "$SEED_COUNT" -lt 30 ]; then
  log "   ❌ Required seed data missing in $APP_DIR/data (found: $SEED_COUNT)"
  exit 1
fi
if [ "$BUNDLED_PUBLIC_COUNT" -gt 0 ]; then
  if [ "$STYLE_COUNT" -lt 5 ]; then
    log "   ❌ Required gamma image styles missing in $UPLOAD_DIR/public/gamma/image-styles (found: $STYLE_COUNT)"
    exit 1
  fi
  if [ "$THEME_COUNT" -lt 50 ]; then
    log "   ❌ Required gamma theme thumbnails missing in $UPLOAD_DIR/public/gamma/themes (found: $THEME_COUNT)"
    exit 1
  fi
  if [ "$BRANDING_COUNT" -lt 1 ]; then
    log "   ❌ Required platform branding missing in $UPLOAD_DIR/public/branding/platform"
    exit 1
  fi
else
  if [ "$STYLE_COUNT" -lt 5 ] || [ "$THEME_COUNT" -lt 50 ] || [ "$BRANDING_COUNT" -lt 1 ]; then
    log "   ⚠️  Packaged upload seed assets not found; continuing with empty/default runtime uploads"
    log "   ⚠️  Asset counts (styles=$STYLE_COUNT, themes=$THEME_COUNT, branding=$BRANDING_COUNT)"
  fi
fi

if [ "$MANIFEST_MISSING" = "true" ]; then
  log "   ✅ Runtime asset validation passed with generated fallback manifest (seed=$SEED_COUNT, styles=$STYLE_COUNT, themes=$THEME_COUNT, branding=$BRANDING_COUNT)"
else
  log "   ✅ Runtime asset validation passed (seed=$SEED_COUNT, styles=$STYLE_COUNT, themes=$THEME_COUNT, branding=$BRANDING_COUNT)"
fi
...
# ============================================
# Step 7b: Set Up Master Password (for SuperAdmin role assignment via lppadmin)
# ============================================
# Auto-generated from the platform SuperAdmin password — no user prompt needed.
log "🔑 Setting up master password for lppadmin..."
...
ORG_NAME="${LEARNPLAY_ORG_NAME:-}"
if [ -z "$ORG_NAME" ]; then
  if is_non_interactive; then
    log "   ❌ LEARNPLAY_ORG_NAME is required in non-interactive mode"
    exit 1
  fi
  ORG_NAME="My Organization"
fi
if [ -n "${LEARNPLAY_TIMEZONE:-}" ]; then
  ORG_TIMEZONE="$LEARNPLAY_TIMEZONE"
else
  ORG_TIMEZONE="$(timedatectl show -p Timezone --value 2>/dev/null | tr -d '[:space:]' || true)"
  if [ -z "$ORG_TIMEZONE" ] && [ -f /etc/timezone ]; then
    ORG_TIMEZONE="$(head -n 1 /etc/timezone 2>/dev/null | tr -d '[:space:]' || true)"
  fi
  ORG_TIMEZONE="${ORG_TIMEZONE:-Etc/UTC}"
fi

# Cloud deployments must never assign SuperAdmin or CustSuper to the
# prompted customer admin. Multi-tenant cloud uses platform SuperAdmin
# (admin@learnplay.co.za) plus organization roles only.
IS_CUST_SUPER="false"
IS_SUPER_ADMIN="true"
...
# Hard validation: required platform superadmin must exist and be enabled.
if psql "$DATABASE_URL" -v ON_ERROR_STOP=1 2>&1 <<'EOSQL_SUPERADMIN_VERIFY' | tee -a "$LOG_FILE"
DO $$
DECLARE
  v_count INTEGER;
BEGIN
  ...
  IF v_count = 0 THEN
    RAISE EXCEPTION 'Required SuperAdmin user admin@learnplay.co.za is missing or not enabled';
  END IF;
END
$$;
EOSQL_SUPERADMIN_VERIFY
then
...
# Step 8a.1: Seed required support bootstrap account for cloud tracks.
...
then
  log "   ✅ Support bootstrap user seeded (support@learnplay.co.za, role=superadmin)"
else
...
BEGIN
  ...
  IF v_org_id IS NULL THEN
    INSERT INTO organizations (id, name, "inviteCode", type, "subscriptionStatus", "isDemo", "useOrgCreditWallet", "allowTeachersToSpendCredits", timezone)
    VALUES (
      gen_random_uuid()::text,
      '${SQL_ORG_NAME}',
      '${INVITE_CODE}',
  ...
  ELSE
    ...
    SET "subscriptionStatus" = 'active',
        ...
        "useOrgCreditWallet" = COALESCE("useOrgCreditWallet", true),
        "allowTeachersToSpendCredits" = COALESCE("allowTeachersToSpendCredits", true),
        timezone = COALESCE(timezone, '${SQL_ORG_TIMEZONE}')
    WHERE id = v_org_id;
  END IF;

  -- Assign org_admin role if not already assigned
  IF NOT EXISTS (
    SELECT 1 FROM "userOrganizationRoles"
    WHERE "userId" = v_user_id AND "organizationId" = v_org_id
  ) THEN
...
then
  ...
  if [ "$IS_CUST_SUPER" = "true" ]; then
    log "   ✅ CustSuper role assigned (Mode A)"
  fi
else
  log "   ⚠️  Admin/organization seeding failed — check database connectivity"
fi

# Hard validation: installer must leave a usable first org + org admin login.
log "🧪 Verifying bootstrap user and organization..."
if psql "$DATABASE_URL" -v ON_ERROR_STOP=1 2>&1 <<EOSQL_BOOTSTRAP_VERIFY | tee -a "$LOG_FILE"
DO \$\$
DECLARE
  v_user_id TEXT;
  v_org_id TEXT;
...
BEGIN
  SELECT id INTO v_user_id FROM users WHERE email = '${SQL_ADMIN_EMAIL}' LIMIT 1;
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'Bootstrap verification failed: admin user % not found', '${SQL_ADMIN_EMAIL}';
  END IF;

  SELECT id INTO v_org_id FROM organizations WHERE name = '${SQL_ORG_NAME}' ORDER BY "createdAt" DESC LIMIT 1;
  IF v_org_id IS NULL THEN
    RAISE EXCEPTION 'Bootstrap verification failed: organization % not found', '${SQL_ORG_NAME}';
  END IF;
  ...
  IF v_role_count = 0 THEN
    RAISE EXCEPTION 'Bootstrap verification failed: user % has no org_admin role for organization %', '${SQL_ADMIN_EMAIL}', '${SQL_ORG_NAME}';
  END IF;
END
\$\$;
EOSQL_BOOTSTRAP_VERIFY
then
  log "   ✅ Bootstrap verification passed"
else
  log "   ❌ Bootstrap verification failed"
  exit 1
fi
...
if [ -n "$GEMINI_KEY" ]; then
  ...
  if [ -z "$ADMIN_USER_ID" ]; then
    ADMIN_USER_ID=$(psql "$DATABASE_URL" -tAc "SELECT id FROM users WHERE \"isSuperAdmin\" = true OR \"isCustSuper\" = true ORDER BY \"createdAt\" ASC LIMIT 1" 2>/dev/null || echo '')
  fi
...
if [ -d "$DIST_DIR/data" ] && [ "$(ls -A "$DIST_DIR/data" 2>/dev/null)" ]; then
  ...
  if [ -f "$APP_DIR/scripts/import-platform-data.sh" ]; then
    export DATABASE_URL
    bash "$APP_DIR/scripts/import-platform-data.sh" "$APP_DIR/data" 2>&1 | tee -a "$LOG_FILE"
    log "   ✅ Platform data imported"

  else
    log "   ⚠️  Import script not found, skipping"
  fi
else
  log "   ℹ️  No platform data to import"
fi

PLATFORM_LOGO_FILE="$(find "$UPLOAD_DIR/public/branding/platform" -maxdepth 1 -type f -name 'logo-*.png' 2>/dev/null | sort | tail -1 || true)"
PLATFORM_FAVICON_FILE="$(find "$UPLOAD_DIR/public/branding/platform" -maxdepth 1 -type f -name 'favicon-*.png' 2>/dev/null | sort | tail -1 || true)"
PLATFORM_LOGO_URL=""
PLATFORM_FAVICON_URL=""
if [ -n "$PLATFORM_LOGO_FILE" ]; then
  PLATFORM_LOGO_URL="/api/public/branding/platform/$(basename "$PLATFORM_LOGO_FILE")"
fi
if [ -n "$PLATFORM_FAVICON_FILE" ]; then
  PLATFORM_FAVICON_URL="/api/public/branding/platform/$(basename "$PLATFORM_FAVICON_FILE")"
fi

# Corporate blue baseline theme so first browser access has branded UI.
BRANDING_TOKENS_JSON='{"--primary":"hsl(217 91% 60%)","--primary-foreground":"hsl(0 0% 100%)","--secondary":"hsl(215 28% 17%)","--secondary-foreground":"hsl(0 0% 100%)","--accent":"hsl(213 94% 68%)","--accent-foreground":"hsl(215 28% 17%)","--background":"hsl(210 40% 98%)","--foreground":"hsl(215 28% 17%)","--card":"hsl(0 0% 100%)","--card-foreground":"hsl(215 28% 17%)","--muted":"hsl(214 32% 91%)","--muted-foreground":"hsl(215 16% 35%)","--border":"hsl(214 32% 85%)","--ring":"hsl(217 91% 60%)","--gradient-from":"hsl(217 91% 60%)","--gradient-to":"hsl(213 94% 68%)","--game-primary":"hsl(217 91% 60%)","--game-glow":"hsl(213 94% 68%)"}'
SQL_BRANDING_TOKENS="${BRANDING_TOKENS_JSON//\'/\'\'}"
SQL_PLATFORM_LOGO_URL="${PLATFORM_LOGO_URL//\'/\'\'}"
SQL_PLATFORM_FAVICON_URL="${PLATFORM_FAVICON_URL//\'/\'\'}"

# Ensure baseline branding rows exist without clobbering existing customized theme data.
log "🎨 Ensuring default platform and organization branding..."
if psql "$DATABASE_URL" -v ON_ERROR_STOP=1 2>&1 <<EOSQL_BRANDING | tee -a "$LOG_FILE"
DO \$\$
DECLARE
  v_org_id TEXT;
BEGIN
  SELECT id INTO v_org_id FROM organizations WHERE name = '${SQL_ORG_NAME}' ORDER BY "createdAt" DESC LIMIT 1;
  IF v_org_id IS NULL THEN
    RAISE EXCEPTION 'Branding bootstrap failed: organization % not found', '${SQL_ORG_NAME}';
  END IF;

  INSERT INTO "brandingThemes" (
    id, "organizationId", "orgName", status, tokens, "logoUrl", "faviconUrl", "fontHeading", "fontBody",
    "supportUrl", "supportEmail", "termsUrl", "privacyUrl",
    "allowEmailBranding", "enableContrastCorrections", "customCopy"
  )
  VALUES (
    gen_random_uuid()::text, v_org_id, '${SQL_ORG_NAME}', 'active', '${SQL_BRANDING_TOKENS}'::jsonb, NULLIF('${SQL_PLATFORM_LOGO_URL}', ''), NULLIF('${SQL_PLATFORM_FAVICON_URL}', ''), 'Inter', 'Inter',
    NULL, NULL, NULL, NULL,
      false, true, '{}'::jsonb
  )
  ON CONFLICT ("organizationId") DO UPDATE
    SET "orgName" = EXCLUDED."orgName",
        status = 'active',
        tokens = CASE
          WHEN "brandingThemes".tokens IS NULL OR "brandingThemes".tokens = '{}'::jsonb THEN EXCLUDED.tokens
          ELSE "brandingThemes".tokens
        END,
        "logoUrl" = COALESCE("brandingThemes"."logoUrl", EXCLUDED."logoUrl"),
        "faviconUrl" = COALESCE("brandingThemes"."faviconUrl", EXCLUDED."faviconUrl"),
        "updatedAt" = NOW();
  ...
  IF EXISTS (SELECT 1 FROM "brandingThemes" WHERE "organizationId" IS NULL) THEN
    UPDATE "brandingThemes"
      SET status = 'active',
          "orgName" = 'LearnPlay',
          tokens = CASE
            WHEN "brandingThemes".tokens IS NULL OR "brandingThemes".tokens = '{}'::jsonb THEN '${SQL_BRANDING_TOKENS}'::jsonb
            ELSE "brandingThemes".tokens
          END,
          "logoUrl" = COALESCE("brandingThemes"."logoUrl", NULLIF('${SQL_PLATFORM_LOGO_URL}', '')),
          "faviconUrl" = COALESCE("brandingThemes"."faviconUrl", NULLIF('${SQL_PLATFORM_FAVICON_URL}', '')),
          "updatedAt" = NOW()
  ...
  ELSE
    ...
    VALUES (
      gen_random_uuid()::text, NULL, 'LearnPlay', 'active', '${SQL_BRANDING_TOKENS}'::jsonb, NULLIF('${SQL_PLATFORM_LOGO_URL}', ''), NULLIF('${SQL_PLATFORM_FAVICON_URL}', ''), 'Inter', 'Inter',
      NULL, NULL, NULL, NULL,
      false, true, '{}'::jsonb
    );
  END IF;
...
EOSQL_BRANDING
then
  log "   ✅ Branding bootstrap complete"
else
  log "   ❌ Branding bootstrap failed"
  exit 1
fi

log "🧪 Verifying seeded non-transactional baseline data..."
if ! verify_non_transactional_seed_data; then
  exit 1
fi
...
if [ -f "$DIST_DIR/nginx.conf.template" ]; then
  ...
  if nginx -t 2>&1; then
    ...
    log "   ℹ️  To enable HTTPS later, run: sudo lppadmin ssl"
  else
...
if [ "$SSL_SETUP" = "caddy-http" ]; then
  ...
  log "   ✅ Environment set for HTTPS at proxy and HTTP backend on this host"
...
if [ -f "$APP_DIR/scripts/lppadmin.sh" ]; then
  ...
  /usr/local/bin/lppadmin cloud self-check >/dev/null 2>&1 || true
...
# ============================================
# Step 15: Persist Installed Version Metadata
# ============================================
...
# ============================================
log "🏷️  Writing runtime identity and provenance..."
write_runtime_identity
write_release_provenance
...
log "  API Keys:   LearnPlay-managed (internal cloud)"
