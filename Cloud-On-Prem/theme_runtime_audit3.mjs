...
const outDir = '/antigravity/artifacts/theme-runtime-audit-3';
...
const contrastEval = `(() => {
  ...
  const selectors = 'h1,h2,h3,h4,h5,h6,p,span,button,a,label,td,th,li';
  ...
  return { rootTokens, lowContrastCount: findings.length, worst: findings.slice(0, 25) };
})()`;
...
if (await inputs.count()) {
  ...
  if ((await inputs.count()) > 1) await inputs.nth(1).fill(PASSWORD);
}
...
const routes = ['/theme-editor', '/management-hub', '/org-management', '/org-structure', '/course-builder'];
const report = [];
for (const route of routes) {
  ...
  report.push({ route, url, finalUrl, screenshot: file, ...result });
}
...
for (const entry of report) {
  console.log(`${entry.route} final=${entry.finalUrl} lowContrast=${entry.lowContrastCount}`);
}
