...
export default function CardsManager() {
  ...
  const handleEdit = (card: any) => {
    ...
    if (card.stats && statTypes && Array.isArray(statTypes)) {
      (statTypes as any[]).forEach((statType: any) => {
        const existingStat = card.stats.find((s: any) => s.statTypeId === statType.id);
  ...
  const selectedCollectionData = (collections as any[])?.find((c: any) => c.id === selectedCollection);

  const handleImageUpload = async (card: any) => {
    try {
      const response: any = await apiRequest(`/api/admin/cards/${card.id}/image-upload-url`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  const handleImageUploadComplete = (card: any) => {
  ...
  return (
    <QuizAdminLayout title="Card Management" description="Create and manage individual cards within collections" activeSection="cards">
        ...
        <div className="flex justify-between items-center">
          ...
          {selectedCollection && (
            <Dialog open={isCreateDialogOpen} onOpenChange={(open) => {
              ...
              if (open) {
                const nextDisplayOrder = cards && (cards as any[]).length > 0 
                  ? Math.max(...(cards as any[]).map((card: any) => card.displayOrder)) + 1 
                  : 1;
                createCardForm.reset({
                  name: "",
                ...
                <form onSubmit={createCardForm.handleSubmit(handleCreate)} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder={`Next available: ${cards && (cards as any[]).length > 0 ? Math.max(...(cards as any[]).map((card: any) => card.displayOrder)) + 1 : 1}`}
                    />
                  </div>
                  <DialogFooter>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {(cards as any[]).map((card: any) => (
                  <Card key={card.id} className="bg-card border-border hover:border-primary/30 transition-all duration-300" data-testid={`card-${card.name.toLowerCase().replace(/\s+/g, '-')}`}>
                    ...
                    <CardContent>
                      {/* Card Image Preview */}
                      <div className="mb-4 h-32 bg-muted rounded-lg flex items-center justify-center border border-border">
                        {card.imageKey ? (
                          <img 
                        ...
                        <div className="mb-4 space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {card.stats.map((stat: any) => (
                              <div key={stat.id} className="bg-muted rounded px-2 py-1">
                      ...
                      <div className="space-y-2">
                        <div className="flex space-x-2">
                          ...
                          <Button onClick={() => handleDelete(card.id)}
                            variant="outline"
                            size="sm"
                            className="bg-destructive/10 border-[var(--destructive)]/30 text-destructive hover:bg-destructive/20"
        ...
        <Dialog open={!!editingCard} onOpenChange={(open) => !open && setEditingCard(null)}>
          <DialogContent className="bg-card border-border text-foreground w-[95vw] sm:w-full max-w-sm sm:max-w-md lg:max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto pr-2">
              ...
              {/* Stat Fields */}
              {statTypes && Array.isArray(statTypes) && (statTypes as any[]).length > 0 ? (
                <div className="space-y-3">
                  <h3 className="text-sm font-medium text-muted-foreground">Card Statistics</h3>
                  <p className="text-xs text-muted-foreground">Enter the values for each stat. You can use decimals (e.g., 3.5).</p>
                  {(statTypes as any[]).map((statType: any) => (
                    <div key={statType.id} className="space-y-1">
                      <Label htmlFor={`stat_${statType.id}`} className="text-sm">
                        {statType.statName}
                        {statType.statUnit && ` (${statType.statUnit})`}
                      </Label>
                      <Input
                        id={`stat_${statType.id}`}
                        type="text"
                        {...editCardForm.register(`stat_${statType.id}` as any)}
                        placeholder={`Enter ${statType.statName.toLowerCase()} value`}
                        className="bg-muted border-border text-foreground"
                        data-testid={`input-edit-stat-${statType.id}`}
                      />
                    </div>
                  ))}
                </div>
            ...
            <DialogFooter className="flex-shrink-0">
              <Button type="submit" form="edit-card-form" disabled={updateMutation.isPending} data-testid="button-submit-edit-card" >
