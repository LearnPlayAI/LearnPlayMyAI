...
import { Plus, Edit2, Trash2, Database, X, ImageIcon } from "lucide-react";
...
export default function CollectionsManager() {
  ...
  const handleEdit = async (collection: any) => {
    ...
    try {
      const statTypes: any = await apiRequest(`/api/admin/collections/${collection.id}/stat-types`);
      editCollectionForm.reset({
        name: collection.name,
        description: collection.description || "",
        isActive: collection.isActive,
        statTypes: statTypes && statTypes.length > 0 ? statTypes : [{ statName: "", statUnit: "", displayOrder: 1 }],
      });
    } catch (error) {
  ...
  const handleToggleActive = (collection: any) => {
  ...
  const removeStatType = (form: any, index: number) => {
    const currentStatTypes = form.watch("statTypes");
    if (currentStatTypes.length > 1) {
      const updatedStatTypes = currentStatTypes.filter((_: any, i: number) => i !== index);
      const reorderedStatTypes = updatedStatTypes.map((stat: any, i: number) => ({
        ...stat,
        displayOrder: i + 1
      }));
  ...
  const handleCollectionCoverImageUpload = async () => {
    ...
    try {
      const response: any = await apiRequest(`/api/admin/collections/${editingCollection.id}/cover-image/upload-url`, {
        method: "POST",
      });
      return {
        method: "PUT" as const,
  ...
  const handleCollectionCoverImageComplete = async (result: any) => {
  ...
  return (
    <QuizAdminLayout
      ...
      description={`Create and manage card collections for the ${orgName} game`}
              ...
              <form onSubmit={createCollectionForm.handleSubmit(handleCreate)} className="space-y-4">
                ...
                <div className="space-y-4 border-t border-border pt-4">
                  ...
                  <div className="space-y-3">
                    {(createCollectionForm.watch("statTypes") || [])?.map((_: any, index: number) => (
                      <div key={index} className="flex gap-2 items-end">
                        ...
                        {(createCollectionForm.watch("statTypes") || []).length > 1 && (
                          <Button type="button" onClick={() => removeStatType(createCollectionForm, index)}
                            variant="outline"
                            size="sm"
                            className="bg-destructive/10 border-[var(--destructive)]/30 text-destructive hover:bg-destructive/20 p-2"
          ...
          <DialogContent className="bg-card border-border text-foreground w-[95vw] sm:w-full max-w-sm sm:max-w-md lg:max-w-xl xl:max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={editCollectionForm.handleSubmit(handleUpdate)} className="space-y-4">
              ...
              <div className="space-y-4 border-t border-border pt-4">
                ...
                <div className="space-y-2">
                  <ObjectUploader
                    ...
                    buttonClassName="w-full bg-primary hover:bg-primary/90 text-btn-primary-foreground"
                  >
              ...
              <div className="space-y-4 border-t border-border pt-4">
                ...
                <div className="space-y-3">
                  {(editCollectionForm.watch("statTypes") || [])?.map((_: any, index: number) => (
                    <div key={index} className="flex gap-2 items-end">
                      <div className="flex-1 space-y-1">
                        <Label className="text-xs">Stat Name</Label>
                        <Input
