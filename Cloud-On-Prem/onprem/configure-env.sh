...
SENSITIVE_KEYS="SMTP_PASS|MAILERSEND_API_KEY|GEMENI_API_KEY|GEMINI_API_KEY|GAMMA_API_KEY|SESSION_SECRET|DATABASE_URL|PASSWORD_RESET_SECRET|EMAIL_VERIFICATION_SECRET|MASTER_PASSWORD_HASH|LEARNPLAY_MASTER_KEY"
...
is_sensitive_key() {
  ...
  if echo "$key" | grep -qE "(_SECRET|_TOKEN|_PASSWORD|_PASS|_API_KEY|PRIVATE_KEY)$"; then
    return 0
  fi
  return 1
}
  ...
  if [ "$WAS_ENCRYPTED" = true ]; then
    if [ "$CONFIG_CHANGED" = true ]; then
      ...
      if [ "$healthy" = true ]; then
        ...
        msg_warn "App may need manual restart: sudo lpadmin → 9 → 3 (Start app with encrypted secrets)"
      fi
    fi
