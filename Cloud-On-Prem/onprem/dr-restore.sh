    ...
    if [ "$DOMAIN_CHANGED" = true ]; then
      ...
      if [ "$FLAG_YES" != true ]; then
        ...
        if [ "$skip_ssl" != "n" ] && [ "$skip_ssl" != "N" ]; then
          warn "SSL certificate restore skipped — run 'sudo lpadmin ssl' to get new certs"
...
phase_7_start_services() {
  ...
  if [ "$app_started" != true ]; then
    local service_unit=""
    for candidate in learnplay-onprem.service learnplay.service; do
  ...
  if [ "$health_ok" = true ]; then
    ...
    if [ "$app_runtime" = "pm2" ]; then
      ...
      warn "Check logs: journalctl -u learnplay-onprem.service -n 100 --no-pager"
    fi
  fi
