...
DEFAULT_APP_DIR="/opt/learnplay/onprem"
[ -d "$DEFAULT_APP_DIR" ] || DEFAULT_APP_DIR="/opt/learnplay"
APP_DIR="${APP_DIR:-$DEFAULT_APP_DIR}"
DATA_DIR="${1:-$APP_DIR/data}"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: bash onprem/import-platform-data.sh [data_dir] [--dry-run] [--allow-nonempty]
...
# If a data directory is explicitly provided, infer APP_DIR from its parent.
if [ -n "${DATA_DIR:-}" ]; then
  inferred_app_dir="$(cd "$(dirname "$DATA_DIR")" 2>/dev/null && pwd -P || true)"
  if [ -n "$inferred_app_dir" ] && [ -f "$inferred_app_dir/.env" ]; then
    APP_DIR="$inferred_app_dir"
  fi
fi
...
BEGIN
  ...
  LOOP
    ...
    IF has_natural_keys AND natural_keys_in_seed THEN
      ...
      natural_set_clause := '';
      FOR col_idx IN 1..array_length(cols, 1)
      LOOP
        col := cols[col_idx];
        IF col = ANY(natural_key_cols) THEN
          CONTINUE;
        END IF;
        IF natural_set_clause != '' THEN
          natural_set_clause := natural_set_clause || ', ';
        END IF;
        natural_set_clause := natural_set_clause || format('%I = %s', col, col_literals[col_idx]);
      END LOOP;

      IF natural_set_clause != '' THEN
        upsert_sql := format('UPDATE %I SET %s WHERE %s', '$TABLE', natural_set_clause, natural_match_clause);
        EXECUTE upsert_sql;
      END IF;
...
if [ -z "${UPLOAD_DIR:-}" ] && [ -f "$ENV_FILE" ]; then
  UPLOAD_DIR="$(grep -E '^UPLOAD_DIR=' "$ENV_FILE" 2>/dev/null | tail -1 | cut -d= -f2- | tr -d '"' | tr -d "'" || true)"
fi
UPLOAD_DIR="${UPLOAD_DIR:-$APP_DIR/uploads}"
ASSET_SUMMARY_FILE="$DATA_DIR/asset-export-summary.json"
...
if [ -d "$ASSETS_DIR" ] && [ "$(ls -A "$ASSETS_DIR" 2>/dev/null)" ]; then
  ...
  if [ ! -s "$ASSET_SUMMARY_FILE" ]; then
    echo "❌ Missing required asset summary: $ASSET_SUMMARY_FILE"
    exit 1
  fi
  ...
  if [ "$ASSET_MISSING_COUNT" -ne 0 ]; then
    echo "❌ Package asset integrity check failed: missing=$ASSET_MISSING_COUNT (must be 0)"
    exit 1
  fi
  if [ "$ASSET_COPIED_COUNT" -lt "$ASSET_REF_COUNT" ]; then
    echo "❌ Package asset integrity check failed: copied=$ASSET_COPIED_COUNT refs=$ASSET_REF_COUNT"
    exit 1
  fi

  STAGED_COUNT="$(find "$ASSETS_DIR" -type f | wc -l | tr -d ' ')"
  if [ "$STAGED_COUNT" -lt "$ASSET_COPIED_COUNT" ]; then
    echo "❌ Staged assets are incomplete: staged=$STAGED_COUNT expected>=$ASSET_COPIED_COUNT"
    exit 1
  fi
  ...
  while read -r asset_file; do
    ...
    cp "$asset_file" "$dest"
    if [ ! -f "$dest" ]; then
      DEST_MISSING=$((DEST_MISSING + 1))
    fi
  done < <(find "$ASSETS_DIR" -type f)

  if [ "$DEST_MISSING" -ne 0 ]; then
    echo "❌ Failed to copy all bundled assets to uploads: missing_in_dest=$DEST_MISSING"
    exit 1
  fi

  if [ "$EUID" -eq 0 ] 2>/dev/null; then
    chown -R "$APP_USER:$APP_USER" "$UPLOAD_DIR/public/"
  fi

  echo "   ✅ Image assets copied to $UPLOAD_DIR/public/ (staged=$STAGED_COUNT, refs=$ASSET_REF_COUNT)"
else
