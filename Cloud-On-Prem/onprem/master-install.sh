...
#   LEARNPLAY_EMAIL_METHOD    — Email provider selection (smtp|mailersend)
#   LEARNPLAY_MODE            — Operating mode (always "A" — customer-provided keys)
...
#   LEARNPLAY_OS_PREP_MODE    — Phase 1 behavior (minimal only)
#   Integration secrets (SMTP/MailerSend/Gemini/Gamma/ElevenLabs/YOCO) are managed after install in /admin/integration-settings
#   LEARNPLAY_ORG_NAME        — Organization name (required)
#   LEARNPLAY_ADMIN_PASSWORD  — Admin password (required)
#   LEARNPLAY_SETUP_SSL       — SSL mode flag (self-signed-https|letsencrypt-https|caddy-http)
#   LEARNPLAY_DIST_DIR        — Path to dist-onprem distribution package
#
# Usage:
...
LEARNPLAY_OS_PREP_MODE="minimal"
...
validate_required_directories() {
  ...
  local pg_target_dir="${LEARNPLAY_PGDATA:-/opt/lpdb/onprem/pg16/main}"
...
save_state() {
  ...
  local vars=(
    ...
    LEARNPLAY_SMTP_FROM LEARNPLAY_EMAIL_METHOD LEARNPLAY_MODE LEARNPLAY_GEMINI_KEY LEARNPLAY_GAMMA_KEY
    ...
    LEARNPLAY_DIST_DIR LEARNPLAY_OS_PREP_MODE
  )
  for var in "${vars[@]}"; do
    local val="${!var:-}"
    val="${val//\'/\'\\\'\'}"
...
stage_dist_package() {
  ...
  for item in server client shared data migrations scripts package.json package-lock.json \
              ...
              schema-contract.env schema-fingerprint.env nginx.conf.template \
...
─── Configuration ─────────────────────────────────────────────────
  ...
  Integrations:     Configure in Integration Settings UI
  Cloud Sync Auth:  $([ -n "${LEARNPLAY_ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD:-}" ] && echo "Configured" || echo "Not configured")
  System Type:      $(case "${LEARNPLAY_SYSTEM_TYPE:-development}" in production) echo "Production" ;; qa) echo "QA/Testing" ;; *) echo "Development" ;; esac)
  OS Prep Mode:     Minimal (no hardening)
...
─── Phase Results ─────────────────────────────────────────────────
  ...
  Phase 1 (OS Prep):             $([ "${PHASE_1_DONE:-false}" = "true" ] && echo "✅ PASSED" || echo "⬜ NOT RUN")
...
─── Deployment Details ────────────────────────────────────────────
  ...
  Payment Gateway:  Disabled (credits managed by admin)
  Admin Role:       CustSuper

─── Filesystem Layout ─────────────────────────────────────────────

  Application:      ${APP_DIR}/
  Uploads:          ${APP_DIR}/uploads/
  Database:         ${LEARNPLAY_PGDATA:-/opt/lpdb/onprem/pg16/main} (PostgreSQL)
...
echo "╔════════════════════════════════════════════════╗"
echo "║                                                ║"
echo "║  LearnPlay On-Premises Master Installer        ║"
...
ALLOW_RESUME="${LEARNPLAY_ALLOW_RESUME:-false}"
if [ "${1:-}" = "--resume" ]; then
  ALLOW_RESUME="true"
fi

if [ "$ALLOW_RESUME" != "true" ] && [ -f "$STATE_FILE" ]; then
  echo "ℹ️  Ignoring previous installation state. Starting fresh configuration."
  rm -f "$STATE_FILE"
fi
...
if [ "$ALLOW_RESUME" = "true" ] && [ -f "$STATE_FILE" ]; then
  ...
  if [ "$COMPLETED" -gt 0 ] && [ "$COMPLETED" -lt 5 ]; then
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    echo ""
    echo "  Options:"
    echo "    R) Resume from Phase $((COMPLETED+1)) (recommended)"
    echo "    N) Start fresh (re-enter all settings)"
    echo "    Q) Quit"
    echo ""
    read -p "  Choice [R]: " RESUME_CHOICE
    RESUME_CHOICE="${RESUME_CHOICE:-R}"
    RESUME_CHOICE=$(echo "$RESUME_CHOICE" | tr '[:lower:]' '[:upper:]')

    if [ "$RESUME_CHOICE" = "Q" ]; then
      echo "  Exiting."
      exit 0
    elif [ "$RESUME_CHOICE" = "R" ]; then
      RESUME_FROM=$((COMPLETED+1))
      echo ""
      echo "  Resuming from Phase $RESUME_FROM..."
      echo ""
    else
      rm -f "$STATE_FILE"
      RESUME_FROM=1
...
if [ "$RESUME_FROM" -eq 1 ] && [ -z "${LEARNPLAY_DOMAIN:-}" ]; then
  ...
  echo "  1. Prepare OS baseline (no hardening by default)"
...
if [ "$RESUME_FROM" -eq 1 ] && [ -z "${LEARNPLAY_DOMAIN:-}" ]; then
  ...
  while [ "$(echo "$LEARNPLAY_ADMIN_EMAIL" | tr '[:upper:]' '[:lower:]')" = "admin@learnplay.co.za" ]; do
    echo "  ❌ admin@learnplay.co.za is reserved for platform SuperAdmin."
    read -p "  Enter your organization admin email: " LEARNPLAY_ADMIN_EMAIL
  done
  ...
  LEARNPLAY_EMAIL_METHOD="mailersend"
  LEARNPLAY_GEMINI_KEY=""
  LEARNPLAY_GAMMA_KEY=""
  LEARNPLAY_ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD="${LEARNPLAY_ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD:-}"
  if [ -z "$LEARNPLAY_ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD" ]; then
    echo ""
    echo "── Cloud PRD Check-In Signing Secret (Optional) ──"
    echo ""
  ...
  echo ""
  echo "    2) QA/Testing   — For integration/UAT."
  echo "                      Learner registration is always disabled."
  echo ""
  echo "    3) Production   — For live use."
  echo "                      No on-prem license is required on LearnPlay cloud."
  echo ""
  ...
  if [ "$SYSTEM_TYPE_CHOICE" = "3" ]; then
    ...
    LEARNPLAY_SYSTEM_TYPE="qa"
  else
    LEARNPLAY_SYSTEM_TYPE="development"
  fi

  case "$(echo "${LEARNPLAY_SYSTEM_TYPE:-development}" | tr '[:upper:]' '[:lower:]')" in
    development|dev|onprem) LEARNPLAY_SYSTEM_TYPE="development" ;;
    qa|test|testing|quality_assurance|quality-assurance) LEARNPLAY_SYSTEM_TYPE="qa" ;;
    production|prod) LEARNPLAY_SYSTEM_TYPE="production" ;;
    *)
      log "Unknown LEARNPLAY_SYSTEM_TYPE '${LEARNPLAY_SYSTEM_TYPE}', defaulting to development"
      LEARNPLAY_SYSTEM_TYPE="development"
      ;;
  esac
  ...
  echo "  Email:         Configure in /admin/integration-settings"
  ...
  echo "  OS Prep:       Minimal (no hardening)"
  echo "  System Type:   $(case "$LEARNPLAY_SYSTEM_TYPE" in production) echo "Production" ;; qa) echo "QA/Testing" ;; *) echo "Development" ;; esac)"
  echo "  Cloud Sync:    $([ -n "$LEARNPLAY_ONPREM_CLOUD_SYNC_SHARED_SECRET_PRD" ] && echo "Configured" || echo "Not configured")"
...
if [ -z "${LEARNPLAY_DIST_DIR:-}" ] || [ ! -d "${LEARNPLAY_DIST_DIR}/server" ]; then
  LEARNPLAY_DIST_DIR="$(resolve_dist_dir)" || {
    ...
    echo ""
    echo "   Enter the path to the dist-onprem directory,"
...
if [ "$RESUME_FROM" -le 1 ]; then
  ...
  log "Running minimal OS prep (no security hardening)..."
  if [ -n "${LEARNPLAY_TIMEZONE:-}" ]; then
    timedatectl set-timezone "$LEARNPLAY_TIMEZONE" 2>/dev/null || log "⚠️  Could not set timezone to $LEARNPLAY_TIMEZONE"
  fi
  set +eo pipefail
  bash "$SCRIPT_DIR/os-prep.sh" --setup-admin 2>&1 | tee -a "$LOG_FILE"
  ...
  log "   ✅ Minimal prep complete (hardening intentionally skipped)"
...
log "    OS Prep:        Minimal (hardening skipped)"
...
log "    Payments:       Disabled (credits managed by admin)"
log "    Admin Access:   CustSuper pricing & credit management"
log ""
